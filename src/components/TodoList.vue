<template>
    <div class="container mx-auto px-4 py-20 flex flex-col flex-wrap">
        <h1 class="font-bold font-hairline text-6xl text-center text-yellow-400">PATITOH</h1>

        <todo-card class="mx-auto mt-4 h-auto w-full max-w-sm">
        <div> <img class="mx-auto" src="https://github.com/GabrielAps18/pruebas/blob/gabrielaps/css/contract%20(1).png?raw=true" alt=""></div>
        <hr>
            <div>
                ...
                <div v-for="(todo, index) in todos" :key="todo.id" class="todo-item">
                    <div class="todo-item-left text-base">
                        <input class="checkbox" type="checkbox" v-model="todo.completed">
                    <div v-if="!todo.editing" @dblclick="editTodo(todo)" class="todo-item-label" :class="{ completed : todo.completed}">{{todo.title}}</div>
                    <input v-else class="todo-item-edit" type="text" v-model="todo.title" @blur="doneEdit(todo)" @keyup.enter="doneEdit(todo)" @keyup.esc="cancelEdit(todo)" v-focus>
                    </div>
                    <img src="https://github.com/GabrielAps18/pruebas/blob/gabrielaps/css/recycle%20(1).png?raw=true" class="remove-item cursor-pointer" @click="removeTodo(index)">
